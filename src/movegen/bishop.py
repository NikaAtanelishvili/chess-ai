...
from src.sliding_moves.sliding_moves import generate_moves_wrapper, initialize_bishop_tables
...
class Bishop:
    ...
    def generate_attacks(square: int, blockers: np.uint64) -> np.uint64:
        ...
        for direction in Bishop.directions:
            attacks |= Bishop._generate_attacks_in_direction(square, blockers, direction)
        return attacks
    ...
    def find_magic_number(square: int, relevant_bits: int, mask: np.uint64) -> np.uint64:
        ...
        for i, occ in Bishop.generate_occupancy_subsets(mask):
            occupancies[i] = occ
            attacks_table[i] = Bishop.generate_attacks(square, occ)
    ...
    def generate_attack_table(square: int, magic: np.uint64, relevant_bits: int, mask: np.uint64) -> np.ndarray:
        ...
        for _, occ in Bishop.generate_occupancy_subsets(mask):
            index = ((occ * magic) & np.uint64(0xFFFFFFFFFFFFFFFF)) >> np.uint64(64 - relevant_bits)
            # Compute the actual rook attacks given 'occ'
            attacks_array[index] = Bishop.generate_attacks(square, occ)
    ...
    def initialize_data() -> None:
        ...
        for square in tqdm(range(64), desc='Initializing the data: MAGICS, ATTACKS, SHIFTS, MASKS'):
            ...
            mask = Bishop.generate_occupancy_mask(square)
            Bishop.MASKS[square] = mask
            ...
            Bishop.SHIFTS[square] = np.uint8(64 - relevant_bits)
            ...
            magic = Bishop.find_magic_number(square, relevant_bits, mask)
            Bishop.MAGICS[square] = magic
            ...
            attacks = Bishop.generate_attack_table(square, magic, relevant_bits, mask)
            Bishop.ATTACKS[square] = attacks
    ...
    def generate_move(board: chess.Board, color: bool):
        ...
        raw_moves = generate_moves_wrapper(pieces, occupancy, own_occ, 0)
    ...
    @staticmethod
    def save_data():
        magics_fn = f"bishop_magics"
        attacks_fn = f"bishop_attacks"
        shifts_fn = f"bishop_shifts"
        masks_fn = f"bishop_masks"
        ...
        # Save magic numbers
        np.save(os.path.join(magics_dir, magics_fn), np.array(Bishop.MAGICS, dtype=np.uint64))

        # Save shifts
        np.save(os.path.join(shifts_dir, shifts_fn), np.array(Bishop.SHIFTS, dtype=np.uint8))

        # Save masks
        np.save(os.path.join(masks_dir, masks_fn), np.array(Bishop.MASKS, dtype=np.uint64))

        # Convert attack tables into a dictionary
        attack_dict = {f"square_{sq}": np.array(Bishop.ATTACKS[sq], dtype=np.uint64) for sq in range(64)}
    ...
    def load_data():
        data_files = {
            'magic_numbers': f"../data/magics/bishop_magics.npy",
            'attack_table': f"../data/attacks/bishop_attacks.npz",
            'shifts': f"../data/shifts/bishop_shifts.npy",
            'masks':f"../data/masks/bishop_masks.npy"}
        ...
        if missing_files:
            ...
            if ans == "y":
                Bishop.initialize_data()
            else:
                return  # Exit the function if the user declines

        # Load magic numbers
        try:
            Bishop.MAGICS = np.load(data_files["magic_numbers"]).astype(np.uint64)
        ...
        # Load shifts
        try:
            Bishop.SHIFTS = np.load(data_files["shifts"]).astype(np.uint8)
            print("Shifts loaded")
        except Exception as e:
        ...
        # Load masks
        try:
            Bishop.MASKS = np.load(data_files["masks"]).astype(np.uint64)
            print("Masks loaded")
        except Exception as e:
        ...
        try:
            ...
            with np.load(data_files["attack_table"]) as attack_data:
                ...
                Bishop.ATTACKS_1D = np.zeros(64 * MAX_ENTRIES, dtype=np.uint64)

                for i in range(64):
                    arr = attacks_dict[i]
                    # Copy the attack bitboards into the corresponding block
                    Bishop.ATTACKS_1D[i * MAX_ENTRIES: i * MAX_ENTRIES + len(arr)] = arr
        ...
        initialize_bishop_tables(Bishop.MAGICS, Bishop.MASKS, Bishop.SHIFTS, Bishop.ATTACKS_1D)
